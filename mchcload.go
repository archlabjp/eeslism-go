...
func Hclelm(Nhcload int, Hcload []HCLOAD) {
	for i := 0; i < Nhcload; i++ {
		hc := &Hcload[i]
		// 湿りコイルの場合
		if hc.Wet == 'y' {
...
func Hcldcfv(Nhcload int, _Hcload []HCLOAD) {
	var f0, f1 float64
	...
	for i := 0; i < Nhcload; i++ {
		...
		if Eo.Control != OFF_SW {
			...
			Eo.Co = 0.0
			Eo.Coeffin[0] = -Hcload.CGa
		}

		Eo = Hcload.Cmp.Elouts[1]
		if Eo.Control != OFF_SW {
			if Hcload.Wetmode == 'y' {
				Eo.Coeffo = 1.0
				Eo.Co = f0
				...
				Eo.Coeffo = Hcload.Ga
				Eo.Co = 0.0
				Eo.Coeffin[0] = -Hcload.Ga
				Eo.Coeffin[1] = 0.0
			}
		}

		if Hcload.Type == 'W' {
			Eo = Hcload.Cmp.Elouts[2]
			if Eo.Control != OFF_SW {
				Hcload.Gw = Eo.G
				...
				Eo.Coeffin[0] = -Hcload.CGw
				Eo.Coeffin[1] = -Hcload.CGa
				Eo.Coeffin[2] = -rGa

				if Hcload.Wetmode == 'y' && Hcload.Chmode == COOLING_SW {
					Eo.Coeffin[3] = Hcload.CGa + rGa*f1
					Eo.Coeffin[4] = 0.0
					Eo.Co = -rGa * f0
				} else {
					Eo.Coeffin[3] = Hcload.CGa
					Eo.Coeffin[4] = rGa
					Eo.Co = 0.0
				}
			}
...
func Hcldene(Nhcload int, _Hcload []HCLOAD, LDrest *int, Wd *WDAT) {
	var rest int
	var elo *ELOUT
	ro := 0.0
	ca := 0.0
...
func hclddyint(Nhcload int, _Hcload []HCLOAD) {
	for i := 0; i < Nhcload; i++ {
		Hcload := &_Hcload[i]
...
func hcldmonint(Nhcload int, _Hcload []HCLOAD) {
	for i := 0; i < Nhcload; i++ {
		Hcload := &_Hcload[i]
...
func hcldday(Mon, Day, ttmm, Nhcload, Nday, SimDayend int, _Hcload []HCLOAD) {
	Mo := Mon - 1
	tt := ConvertHour(ttmm)
...
func hclddyprt(fo *os.File, id, Nhcload int, _Hcload []HCLOAD) {
	...
	default:
		for i := 0; i < Nhcload; i++ {
			...
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdys.Hhr, Hcload.Qdys.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdys.Chr, Hcload.Qdys.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.Qdys.Hmxtime, Hcload.Qdys.Hmx)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.Qdys.Cmxtime, Hcload.Qdys.Cmx)
			...
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdyl.Hhr, Hcload.Qdyl.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdyl.Chr, Hcload.Qdyl.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.Qdyl.Hmxtime, Hcload.Qdyl.Hmx)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.Qdyl.Cmxtime, Hcload.Qdyl.Cmx)

			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdyt.Hhr, Hcload.Qdyt.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.Qdyt.Chr, Hcload.Qdyt.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.Qdyt.Hmxtime, Hcload.Qdyt.Hmx)
...
func hcldmonprt(fo *os.File, id, Nhcload int, _Hcload []HCLOAD) {
	...
	default:
		for i := 0; i < Nhcload; i++ {
			...
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdys.Hhr, Hcload.mQdys.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdys.Chr, Hcload.mQdys.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.mQdys.Hmxtime, Hcload.mQdys.Hmx)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.mQdys.Cmxtime, Hcload.mQdys.Cmx)

			fmt.Fprintf(fo, "%1ld %5.4f %1ld %5.4f %1ld %5.4f ",
				Hcload.mxaidy.Hrs, Hcload.mxaidy.M,
				Hcload.mxaidy.Mntime, Hcload.mxaidy.Mn,
				Hcload.mxaidy.Mxtime, Hcload.mxaidy.Mx)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdyl.Hhr, Hcload.mQdyl.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdyl.Chr, Hcload.mQdyl.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.mQdyl.Hmxtime, Hcload.mQdyl.Hmx)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.mQdyl.Cmxtime, Hcload.mQdyl.Cmx)

			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdyt.Hhr, Hcload.mQdyt.H)
			fmt.Fprintf(fo, "%1ld %3.1f ", Hcload.mQdyt.Chr, Hcload.mQdyt.C)
			fmt.Fprintf(fo, "%1ld %2.0f ", Hcload.mQdyt.Hmxtime, Hcload.mQdyt.Hmx)
