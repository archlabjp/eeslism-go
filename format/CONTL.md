...
SYSCMPで定義した機器の内部変数を、条件式や設定式で参照できます。変数名は `機器名_変数名` の形式で指定します。
...
EXSRFで定義した傾斜面の日射量を参照できます。

| 変数名 | 説明 | 単位 |
|:-------|:-----|:-----|
| `傾斜面名_Idre` | 傾斜面入射直達日射量 | W/m² |
| `傾斜面名_Idf` | 傾斜面入射天空日射量 | W/m² |
| `傾斜面名_Iw` | 傾斜面入射全日射量 | W/m² |
| `傾斜面名_Te` | 傾斜面相当外気温度 | ℃ |
...
|:-------|:-----|:-----|
| `機器名_Tout` | 出口温度 | ℃ |
| `機器名_hout` | 出口エンタルピ | J/kg |
| `機器名_xout` | 出口絶対湿度 | kg/kg |
| `機器名_RHout` | 出口相対湿度 | % |
...
| 変数名 | 説明 | 単位 |
|:-------|:-----|:-----|
| `機器名_Ts_t` | タンク上部（トップ）温度 | ℃ |
| `機器名_Ts_b` | タンク下部（ボトム）温度 | ℃ |
| `機器名_Ts_<n>` | タンクn層目温度（0から開始） | ℃ |

**使用例:**
```
CONTL
    ! タンク上部温度が60℃を超えたら蓄熱停止
    if (Tank_Ts_t > 60) ChargePath=OFF ;

    ! タンク下部温度との差で制御
    if (Tank_Ts_t-Tank_Ts_b > 20) ChargePath=OFF ;
*
```
...
CONTL
    ! 蓄熱材温度が600℃を超えたら蓄熱停止
    if (StorageHeater_Ts > 600) HeaterPath=OFF ;
*
```
...
| 変数名 | 説明 | 単位 |
|:-------|:-----|:-----|
| `機器名_value` | バルブ開度 | 0.0〜1.0 |

**使用例:**
```
CONTL
    ! バルブ開度が90%を超えたら警告
    if (HotValv_value > 0.9) WarningMode=ON ;
*
```
...
CONTL
    ! デシカント材湿度が高くなったら再生運転
    if (Desiccant_RH > 80) RegenerationPath=ON ;
*
```
...
```
CONTL
    ! 集熱器温度とタンク下部温度の差で集熱制御
    if (Collector_Te-Tank_Ts_b > 10) CollectorPath=ON ;
*
```
...
CONTL
    ! 室温が26℃を超えたら冷房運転
    if (Office_Tr > 26) CoolingPath=ON ;
*
```
...
CONTL
    ! 床パネル温度が25℃以下なら暖房継続
    if (FloorPanel_Ts < 25) HeatingPath=ON ;
*
```

#### 汎用出口変数

多くの機器で以下の汎用出口変数が使用できます：
...
| 変数名 | 説明 | 値 |
|:-------|:-----|:---|
| `機器名_control` | 機器制御状態 | ON/OFF |
| `機器名_chmode` | 運転モード | HEAT/COOL |
