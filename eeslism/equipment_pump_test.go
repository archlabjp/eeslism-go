...
func testPumpPartLoadCharacteristics(t *testing.T) {
	...
	pumpca := &PUMPCA{
		...
		Go:     0.2,
		qef:    0.12,
		val:    []float64{0.1, 0.9, 0.0}, // 2次式係数 a0 + a1*x + a2*x^2
	}
...
func testPumpControlTypes(t *testing.T) {
	...
	controlTypes := []struct {
	...
	}{
		{
			controlType: "C",
			description: "定流量制御",
			characteristics: []float64{1.0, 0.0, 0.0}, // 一定動力
		},
		{
			controlType: "P",
			description: "太陽電池駆動",
			characteristics: []float64{0.0, 1.0, 0.0}, // 流量比例
		},
	}
