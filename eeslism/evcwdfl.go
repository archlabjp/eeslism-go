...
func wdflinit(Simc *SIMCONTL, Estl *ESTL, Tlist []TLIST) {
	...
	Simc.Wdpt = NewWDPT(Estl, Tlist)
}

func NewWDPT(Estl *ESTL, Tlist []TLIST) WDPT {
	var s string
	var wp WDPT
	wp.Ta = nil
	wp.Xa = nil
	...
	for i := 0; i < Estl.Ndata; i++ {
		...
		if t.Name == "Wd" {
			...
			case "RN": // 夜間放射量
				wp.Rn = val
			default:
				panic("Unknown TLIST.Id. TLIST.id allowed only 'T', 'x', 'Idn', 'Isky', 'Ihor', 'CC', 'Wdre', 'Wv', 'RH', 'RN'")
			}
		} else {
			panic("Unknown TLIST.Name. TLIST.NAME allowed only 'Wd'")
		}
	}
	return wp
}
