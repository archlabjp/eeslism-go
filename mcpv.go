...
func PVdyprt(fo *os.File, id int, Npv int, PV []PV) {
	...
	default:
		for i := 0; i < Npv; i++ {
			fmt.Fprintf(fo, "%1ld %3.1f ", PV[i].Edy.Hhr, PV[i].Edy.H)
			fmt.Fprintf(fo, "%1ld %2.0f\n", PV[i].Edy.Hmxtime, PV[i].Edy.Hmx)

			fmt.Fprintf(fo, "%1ld %3.1f ", PV[i].Soldy.Hrs, PV[i].Soldy.D)
...
func PVmonprt(fo *os.File, id int, Npv int, PV []PV) {
	...
	default:
		for i := 0; i < Npv; i++ {
			fmt.Fprintf(fo, "%1ld %3.1f ", PV[i].mEdy.Hhr, PV[i].mEdy.H)
			fmt.Fprintf(fo, "%1ld %2.0f\n", PV[i].mEdy.Hmxtime, PV[i].mEdy.Hmx)

			fmt.Fprintf(fo, "%1ld %3.1f ", PV[i].mSoldy.Hrs, PV[i].mSoldy.D)
